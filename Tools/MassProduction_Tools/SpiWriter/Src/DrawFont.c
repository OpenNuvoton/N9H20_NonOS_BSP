...
void Draw_InitialBorder(S_DEMO_FONT* ptFont)
{
    ...
    DemoFont_Border(ptFont, &s_sDemo_Rect, 2);
}
...
/***********************************************************************************
  Draw_FinalStatus :
    It is used to show final status of SpiWriter.
...
void Draw_Init(void)
{
    ...
    /* Draw the Boarder for "N9H20 SpiWriter (..)" */
    s_sRect.u32StartX =0;
    s_sRect.u32StartY = 0;
    ...
    sprintf(Array1, "N9H20 SpiWriter (v%d.%d)",MAJOR_VERSION_NUM, MINOR_VERSION_NUM);
#if 0
    Draw_Font(COLOR_RGB16_WHITE, &s_sDemo_Font,
        (_LCM_WIDTH_ - 32*g_Font_Step)/2, /* 32 Character for "N9H20 SpiWriter (%s" */
        0,
        Array1);
#else
    Draw_Font(COLOR_RGB16_WHITE, &s_sDemo_Font,
        0, /* 32 Character for "N9H20 SpiWriter (%s" */
        0,
        Array1);
#endif

    /* Only draw the Boarder for "Spi: Block * Page * PageSize"
       But, Spi information is delayed to Spi initialization */
    s_sRect.u32StartX =0;
