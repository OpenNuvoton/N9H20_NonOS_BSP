...
INT32 HID_CmdReadPages(CMD_T *pCmd)
{
    ...
    if(u32Pages)
    {
        ...
        spiFlashRead(u32StartPage * SPI_PAGE_SIZE, g_u32ReadWriteSize , (UINT32 *)g_u8PageBuff);
...
void HID_GetOutReport(void)
{
    ...
    if((u8Cmd == HID_CMD_WRITE) &&  (u32PageCnt < u32Pages))
    {
        ...
        if(g_u32BytesInPageBuf >= g_u32ReadWriteSize)
        {
            ...
            spiFlashWrite((u32StartPage + u32PageCnt) * SPI_PAGE_SIZE, g_u32ReadWriteSize, (UINT32 *)g_u8PageBuff);
...
void HID_SetInReport(void)
{
    ...
    if(u8Cmd == HID_CMD_READ)
    {
        ...
        else
        {
            ...
            {
                spiFlashRead((u32StartPage + u32PageCnt) * SPI_PAGE_SIZE, g_u32ReadWriteSize , (UINT32 *)g_u8PageBuff);
                if(g_u32EPA_MXP == EPA_MAX_PKT_SIZE)
                    u32PageCnt+=2;                    
                else        
                {
