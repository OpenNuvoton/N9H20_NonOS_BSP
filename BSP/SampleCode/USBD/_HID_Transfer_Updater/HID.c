...
static UINT8  gTemp[64] __attribute__((aligned(4)));    /* Page buffer to upload/download through HID report */

UINT8  *g_u8PageBuff, *g_u8TestPages,*g_Temp;
...
INT32 HID_CmdEraseBlocks(CMD_T *pCmd)
{
    ...
    sysprintf("Erase command - Block: %d\n", u32StartBlock);
...
INT32 HID_CmdReadPages(CMD_T *pCmd)
{
    ...
    if(u32Pages)
    {
        ...
        spiFlashRead(u32StartPage * HID_PAGE_SIZE, HID_PAGE_SIZE , (UINT32 *)g_u8PageBuff);
...
void HID_GetOutReport(void)
{
    ...
    if((u8Cmd == HID_CMD_WRITE) &&  (u32PageCnt < u32Pages))
    {
        ...
        if(g_u32BytesInPageBuf >= HID_PAGE_SIZE)
        {
            ...
            spiFlashWrite((u32StartPage + u32PageCnt) * HID_PAGE_SIZE, HID_PAGE_SIZE, (UINT32 *)g_u8PageBuff);
            u32PageCnt++;
        ...
        else
        {
          //  sysprintf("HID_SetInReport %d\n",g_u32BytesInPageBuf);
            if(g_u32BytesInPageBuf == 0)
            {
                ...
                //sysprintf("%d ", u32StartPage + u32PageCnt);
                spiFlashRead((u32StartPage + u32PageCnt) * HID_PAGE_SIZE, HID_PAGE_SIZE , (UINT32 *)g_u8PageBuff);
...
void Reset_Handler(VOID)  /* Interrupt OUT handler */
{
    //isConnectToHost = 1;
}
