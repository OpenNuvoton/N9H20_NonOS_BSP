...
short sError = pdFALSE, sCheckVariableToUse;
	...
	for( ;; )
	{
		...
		{
			sysprintf("TickCount=%d, TaskName=%s, takes the semaphore\n", 
			...
			if( sError == pdFALSE )
			{
				sysprintf("TickCount=%d, TaskName=%s, gives the semaphore\n", 
									xTaskGetTickCount(), pcTaskGetTaskName( NULL ));
		...
		{
			sysprintf("TickCount=%d, TaskName=%s, takes the semaphore failed!!\n", 
								xTaskGetTickCount(), pcTaskGetTaskName( NULL ));
